#!/usr/bin/env python3
"""
Script to send all dos-prompts from chatdoctor/dos-prompt/ to vllm server.
"""
import os
import json
import requests
...
DOS_PROMPT_DIR = "../chatdoctor/dos-prompt"
MAX_PROMPTS = 1
...
class CudaTracer:
    ...
    def start(self):
        ...
        def trace_thread():
            ...
            try:
                # Run cuda_trace with sudo and redirect output to CSV file using shell redirection
                # Use shell=True to allow '>' redirection
                cmd = f"sudo {cuda_trace_full_path} --pid {self.pid} > {self.output_path}"
                
                self.process = subprocess.Popen(
                    cmd,
                    shell=True,  # Enable shell to process '>' redirection
                ...
                # Wait a moment for the process to start and write header
                import time
                time.sleep(0.5)  # Brief delay to allow header to be written
...
def main():
    ...
    output_csv = f"dos_{timestamp}.csv"
    ...
    # Get all JSON files from dos-prompt directory
    prompt_files = sorted(glob.glob(os.path.join(DOS_PROMPT_DIR, "*.json")))
    
    if not prompt_files:
        print(f"No JSON files found in {DOS_PROMPT_DIR}")
        return
    ...
    for prompt_file in tqdm(prompt_files, desc="Sending prompts"):
        try:
            ...
            request_id = f"dos-{filename.replace('.json', '')}"
    ...
    if tracer:
        ...
        time.sleep(5)  # Wait 5 seconds for pending operations
